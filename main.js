...
function bis(){
	...
	$.ajax({
		url: 'action.php',
		method: 'POST',
		data: {comment_add:1,Comment:keyword},
		success: function(data){
			$('#comment_bool').html(data);
		}
	})
}
	//$('body').delegate('.add_comment','click',com())
	

	/*
	$('body').delegate('#add_comment','click',function(event){
		event.preventDefault();
		var pid=$(this).attr('pid');
		$.ajax({
			url: 'action.php',
			method: 'POST',
			data: {comment_add:1,pid:pid},
			success: function(data){
